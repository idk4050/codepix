...
func New() (*Config, error) {
	...
	if cmp.Equal(c.InitialState, initialState{}) {
		return nil, errors.New("failed to load initial state config")
	}
...
type userAuth struct {
	...
	MinutesUntilExpiration      time.Duration
...
func (c *userAuth) build() error {
	signingKeyPem, _ := pem.Decode([]byte(c.SigningKeyString))
	...
	c.MinutesUntilExpiration = time.Minute * time.Duration(c.MinutesUntilExpirationInt)
	return nil
}
