...
func TestTranslatedErrorMessages(t *testing.T) {
	...
	require.Error(t, err)
	require.IsType(t, &ut.ErrConflictingTranslation{}, err)
...
func TestTranslatedFieldNames(t *testing.T) {
	...
	require.Error(t, err)
	require.Equal(t, err.Error(), "error: conflicting key 'field-name-User.Details.FirstName' rule 'Unknown' with text 'First name' for locale 'en_US', value being ignored")
